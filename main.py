...
import pandas as pd
import json
from src import create_league, run_league, generate_messages, generate_html
...
def generate_schedule_promoted_relegated(teams, fixtures):
    ...
    """
    messages = pd.DataFrame(columns = ["TYPE", "DIV", "MD", "GM", "MSG", "HTML"])
    promotion = []
    relegation = []
    ...
    for i in range(1, 2):
        for j in range(int(fixtures[i].tail(1)["Matchday"])):
            for k in range(len(teams[create_league.DIVISIONS[i+1]].index) // 2):
                ...
                messages = messages.append({"TYPE":"1 - Match", "DIV":i+1, "MD":j+1, "GM":k+1,"MSG":match_msg, "HTML":match_html}, ignore_index=True)
            standings_msg = generate_messages.generate_standings_msg(create_league.DIVISIONS[i+1], j+1)
            standings_html = generate_html.generate_standings_html()
            messages = messages.append({"TYPE":"2 - Standings", "DIV":i+1, "MD":j+1, "GM":100,"MSG":standings_msg,"HTML":standings_html}, ignore_index=True)
            if (j + 1 != int(fixtures[i].tail(1)["Matchday"])):
                ...
                next_matchday_html = generate_html.generate_next_matchday_html()
                messages = messages.append({"TYPE":"3 - Next Matchday", "DIV":i+1, "MD":j+1, "GM":100,"MSG":next_matchday_msg,"HTML":next_matchday_html}, ignore_index=True)
        ...
        if i + 1 != len(create_league.DIVISIONS.keys()):
            ...
            messages = messages.append({"TYPE":"4 - Relegated", "DIV":i+1, "MD":100, "GM":100,"MSG":relegated_msg,"HTML":relegated_html}, ignore_index=True)
        if i != 0:
            ...
            messages = messages.append({"TYPE":"5 - Promoted", "DIV":i+1, "MD":100, "GM":100,"MSG":promoted_msg,"HTML":promoted_html}, ignore_index=True)
        ...
        messages = messages.append({"TYPE":"6 - Champions", "DIV":i+1, "MD":100, "GM":100,"MSG":champion_msg,"HTML":champion_html}, ignore_index=True)
